...
function LayoutContent({ children, currentPageName }) {
  ...
  const getCompanionName = () => {
    const map = { cat: 'Cozy Cat', dog: 'Playful Dog', orb: 'Magic Orb', robot: 'Friendly Robot' };
    return map[currentUser?.companion_type] || 'Friendly Robot';
  };

  const getPersonalityLabel = () => {
    const labels = { motivational: "🔥 Motivational", calm: "🧘 Calm", witty: "😄 Witty", direct: "⚡ Direct" };
    ...
    <>
      ...
      <div className="min-h-screen flex w-full bg-gradient-to-br from-purple-50 via-teal-50 to-blue-50 dark:from-[#1a1025] dark:via-[#0f1a1f] dark:to-[#0d1220] overflow-x-hidden">
                  ...
                  <div className="mx-3 p-4 bg-gradient-to-br from-purple-100 to-teal-100 dark:from-purple-900/30 dark:to-teal-900/30 rounded-2xl space-y-3">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 bg-white dark:bg-gray-700 rounded-full flex items-center justify-center shadow-md">
                        <span className="text-2xl">{getCompanionEmoji()}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{getCompanionName()}</p>
                        <p className="text-xs text-gray-600 dark:text-gray-400">Working with you</p>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <Button onClick={handleChangeCompanion} variant="outline" size="sm" className="w-full bg-white hover:bg-purple-50 border-purple-200 text-purple-700 font-medium">
                        <RefreshCw className="w-3 h-3 mr-2" />
                        Change Companion
                      </Button>
                      <Button onClick={() => setShowPersonalityModal(true)} variant="outline" size="sm" className="w-full bg-white hover:bg-purple-50 border-purple-200 text-purple-700 font-medium">
        ...
        <main className="flex-1 flex flex-col min-w-0">
          <header
            className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-md border-b border-purple-100 dark:border-gray-700 px-3 sm:px-6 py-4 sticky top-0 z-10"
